...
class IPCLIPB16(TrainerXU):
    ...
    def after_epoch(self): 
        ...
        if do_test:
            curr_result_test1, curr_result_test2, curr_result_test3, curr_result_test4, curr_result_test5, curr_result_test6, curr_result_test7, curr_result_test8 = self.test()
            self.save_model(self.epoch,
                            self.output_dir,
                            model_name="model--{}--{:.2f}-->{:.2f}-->{:.2f}--{:.2f}-->{:.2f}-->{:.2f}-->{:.2f}-->{:.2f}.pth.tar".format(self.epoch, curr_result_test1, curr_result_test2, curr_result_test3, curr_result_test4, curr_result_test5, curr_result_test6, curr_result_test7, curr_result_test8))
    ...
    def test(self):
        ...
        self.evaluator.reset()
        split = "test5"
        test_5_data_loader = self.test_loader_5
        print(f"Evaluate on the *{split}* set")
        for batch_idx, batch in enumerate(tqdm(test_5_data_loader)):
            input, label = self.parse_batch_test(batch)
            output = self.model_inference(input)
            self.evaluator.process(output, label)
        results_test_5 = self.evaluator.evaluate()
        for k, v in results_test_5.items():
            tag = f"{split}/{k}"
            self.write_scalar(tag, v, self.epoch)
        ...
        self.evaluator.reset()
        split = "test6"
        test_6_data_loader = self.test_loader_6
        print(f"Evaluate on the *{split}* set")
        for batch_idx, batch in enumerate(tqdm(test_6_data_loader)):
            input, label = self.parse_batch_test(batch)
            output = self.model_inference(input)
            self.evaluator.process(output, label)
        results_test_6 = self.evaluator.evaluate()
        for k, v in results_test_6.items():
            tag = f"{split}/{k}"
            self.write_scalar(tag, v, self.epoch)
        ...
        self.evaluator.reset()
        split = "test7"
        test_7_data_loader = self.test_loader_7
        print(f"Evaluate on the *{split}* set")
        for batch_idx, batch in enumerate(tqdm(test_7_data_loader)):
            input, label = self.parse_batch_test(batch)
            output = self.model_inference(input)
            self.evaluator.process(output, label)
        results_test_7 = self.evaluator.evaluate()
        for k, v in results_test_7.items():
            tag = f"{split}/{k}"
            self.write_scalar(tag, v, self.epoch)
        ...
        self.evaluator.reset()
        split = "test8"
        test_8_data_loader = self.test_loader_8
        print(f"Evaluate on the *{split}* set")
        for batch_idx, batch in enumerate(tqdm(test_8_data_loader)):
            input, label = self.parse_batch_test(batch)
            output = self.model_inference(input)
            self.evaluator.process(output, label)
        results_test_8 = self.evaluator.evaluate()
        for k, v in results_test_8.items():
            tag = f"{split}/{k}"
            self.write_scalar(tag, v, self.epoch)
        return list(results_test_1.values())[0], list(results_test_2.values())[0], list(results_test_3.values())[0], list(results_test_4.values())[0], list(results_test_5.values())[0], list(results_test_6.values())[0], list(results_test_7.values())[0], list(results_test_8.values())[0]
